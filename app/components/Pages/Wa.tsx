...
export default function ContactWhatsAppPage(): JSX.Element {
  ...
  const sendWhatsApp = (e?: React.MouseEvent | React.FormEvent) => {
    ...
    const message = [
      ...
      `${templateText}`,
      `${via_url}`,
    ].join("%0A"); // using %0A line breaks (encoded)
...
<section id="contact" className="relative pt-36 pb-32 overflow-hidden">
  ...
  <div
    ...
    style={{
      background:
        "linear-gradient(to top, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.0) 20%, rgba(0,0,0,0.0) 100%, rgba(0,0,0,0.6) 100%)",
    }}
  /> 
  <div
    className="absolute inset-0 z-10 pointer-events-none"
    style={{
      background:
        "linear-gradient(to bottom, rgba(0,0,0,0.6) 0%, rgba(0,0,0,0.0) 20%, rgba(0,0,0,0.0) 80%, rgba(0,0,0,0.6) 100%)",
    }}
  /> 
  ...
  <div className="relative z-10 max-w-6xl mx-auto px-4">
    <div className="w-full pb-30">
       ...
       <h2 className="text-4xl font-extrabold text-center text-white flex items-center justify-center gap-3">
        <BsFillTelephoneFill  className="text-[#7c3aed]" />Contact
      </h2>
